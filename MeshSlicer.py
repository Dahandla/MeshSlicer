...
class OBJECT_OT_mesh_slicer(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        layout.prop(self, "plane_location")
    ...
    def execute(self, context):
        ...
        slicing_plane = add_slicing_plane(context, self.plane_location, self.plane_rotation)
        context.view_layer.objects.active = obj
        bpy.ops.object.mode_set(mode='EDIT')
        bm = bmesh.from_edit_mesh(obj.data)
        plane_normal = mathutils.Vector((0, 0, 1))
        rot_matrix = mathutils.Euler(self.plane_rotation, 'XYZ').to_matrix()
        plane_normal.rotate(rot_matrix)
        geom = bm.verts[:] + bm.edges[:] + bm.faces[:]
        bmesh.ops.bisect_plane(bm, geom=geom, plane_co=self.plane_location, plane_no=plane_normal, 
